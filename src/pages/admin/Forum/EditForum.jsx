...
const EditForum = () => {
    ...
    const fetchForumAndReplies = async () => {
      try {
        ...
        const forumResponse = await axios.get(
          `http://localhost:4000/forum/${id}`
        );
        setForum(forumResponse.data);

        // Fetch replies for the forum
  ...
  const handleDeleteReply = async (replyId) => {
    ...
    try {
      await axios.delete(`http://localhost:4000/replies/${replyId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      <form onSubmit={handleUpdate}>
        <div className="mb-4">
          ...
          <input
            ...
            className="w-full p-2 border border-black rounded bg-white text-black" // Black border and text
          />
        </div>
        <div className="mb-4">
        ...
        <tbody>
          {replies.map((reply) => (
            <tr key={reply._id}>
              ...
              <td
                className="py-2 px-4 text-black"
                dangerouslySetInnerHTML={{ __html: reply.userId.name }} // Render user name as HTML (ensure this is safe)
              />
              <td>
                <button
